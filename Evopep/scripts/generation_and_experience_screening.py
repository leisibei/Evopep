...
library_path = r'E:\LS\深度学习\code\code_new_0815\practice-cxy\input'  # 保存生成肽库后的文件的文件夹,需要修改
subset_num = 200000  # 500的意思是分500份文件保存
total_num = 1e9  # 1e9是10的9次方
experience_screen_path = r'E:\LS\深度学习\code\code_new_0815\practice-cxy\input\经验筛选'  # 通过经验筛选肽库后的文件的文件夹,需要修改


# 选中代码后，按 ctrl + / 可以跳过代码(前面有#的都不会运行)
############### 生成肽库 ###############
library = []
for i in tqdm(range(int(1e9)), desc='生成序列ing'):
    if (i + 1) % subset_num == 0:
...
def judging_experience(seq):
    ...
    # positive_charge = (seq.count('K') + seq.count('R') + seq.count('H') - seq.count('D') - seq.count('E') > 0) #去除正电荷经验筛选
    R_proportion = (seq.count('R') / len(seq)) < 0.25
    return amphiphilicity and R_proportion
    #return amphiphilicity and positive_charge and R_proportion


for i in tqdm(range(int(1e9/subset_num)), desc='经验筛选ing'):
    start = time.time()
